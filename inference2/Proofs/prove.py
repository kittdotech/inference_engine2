                ...
                if (bool2 and isdefineable(all_sent[m]) and definiendum != None and \
                    ...
                    if (relat == "J" and pos == 'a') or (relat == "I" and pos == 'n') or (relat == 'H' and pos == 'n') \
                        ...
                        if definition != None and all_sent[m][0] not in def_sent:
                            ...
                            dummy = change_variables(defined, al_def, definition, definiendum, 0, tot_sent, dv_nam,
                                                     idf_var, \
            ...
            if j == 5:
                ...
                if str3 != None and temp_str != str3:
                    ...
                    if def_sent[i][j] not in taken_out:
                        dummy = abb_change2(match_dv, match_type, def_sent, i, idf_var, temp_match, j, gen_var, cnnan,
                                            rename)
                    unmatched2.append([i, j])
                ...
                if not bool1:
                    ...
                    if len(conjuncts) == 1 and len(list3) >= 1:
                        for k in range(len(conjuncts)):
                            ...
                            for j in range(len(list3)):
                                ...
                                use_statement_logic = axioms2(greek2, pos1, pos2, rel1, rel2, sub1, obj1, sub2, obj2,
                                                              osec_sent, tot_sent, used_ax, \
                                                              candd, candd2, conditionals, all_sent, member_prop,
                                                              not_id, prop_sent)
                                if use_statement_logic:
                                    break
                        ...
                        use_statement_logic = axioms2(greek2, pos1, pos2, rel1, rel2, sub1, obj1, sub2, obj2, osec_sent,
                                                      tot_sent, used_ax, \
                                                      candd, candd2, conditionals, all_sent, member_prop, not_id,
                                                      prop_sent)
                        if use_statement_logic:
                            break
                        ...
                        for n in range(y, g - 1):
                            ...
                            while h < g:
                                ...
                                use_statement_logic = axioms2(greek2, pos1, pos2, \
                                                              rel1, rel2, sub1, obj1, sub2, obj2, osec_sent, tot_sent,
                                                              used_ax, \
                                                              candd, candd2, conditionals, all_sent, member_prop,
                                                              not_id, prop_sent)
                                if use_statement_logic:
...
def search_for_category_errors(all_sent, words, basic_object_properties, bo2, \
                               member_prop, not_id, property_sent, tot_sent, detach_sent, \
...
def instantiate(all_sent, tot_sent, basic_object_properties, words, candd, candd2, conditionals, \
    ...
    list1 = rearrange(tot_sent, list1[0], all_sent, greek2, list1[1],
                      idf_var)  # conditionals = list1[1], consistent = list[0]
...
def prepare_prop_sent(prop_sent, tot_sent):
    for sent in reversed(tot_sent):
    ...
    for i in range(len(tot_sent)):
        for j in range(0, 5):
            if len(tot_sent[i]) > 5 + j:
                if tot_sent[i][j + 5] != "":
                    tot_sent[i][j + 5] = dict1.get(tot_sent[i][j + 5], None)

                break

    return [tot_sent, dict1]
...
def print_standard_sent(tot_sent, conditionals, all_sent, consistent, greek2):
    ...
    tot_sent = add_stan_sent(nonstandard, standard_cd, detach_sent, tot_sent)

    return [tot_sent, detach_sent]
        ...
        for j in num:
            if isvariable(detach_sent[i][9][j]):
                ...
                if str1 not in defn and str1 not in indef and str1 not in general:
                    if isinmdlist(detach_sent[i][9][j], dv_nam, 0):
...
def categorize_variables(conditionals, detached_var):
    ...
    return [general, mixed, defn, indef_attach, indef, same_sent]
...
def rearrange(tot_sent, consistent, all_sent, greek2, conditionals, idf_var):
    ...
    conditionals = renumber_conditionals(conditionals, list1[1])  # list1[1] new_numbers
    ...
    if consistent and conditionals != []:
        ...
        detached_var = get_detached_variables(detach_sent)  # list4[0] = tot_sent

        conditionals = link_natural_sent_to_all_sent(conditionals, all_sent)

        variable_type = categorize_variables(conditionals, detached_var)

        tot_sent = print_variables(variable_type, list4[0])

        list3 = get_detached_predicates(variable_type, detach_sent)  # list1[1] detached predicates

        list2 = get_attached_predicates(variable_type, conditionals, list3[0])  # list2[1] = object_properties
        ...
        instantiations = determine_if_same_class(object_properties, idf_var, variable_type[5])

        list5 = use_axiom_of_definition(detach_sent, instantiations, tot_sent)  # list5[0] = tot_sent

        conditionals = substitute_in_conditionals(instantiations, conditionals)  # list5[1] = detach_sent

        tot_sent = print_instantiations(instantiations, list5[0])

        tot_sent = print_new_conditionals(conditionals, tot_sent)

        z = time.time()
        z = z - y
        instan_time += z

    return [tot_sent, conditionals, list5[1]]


def use_axiom_of_definition(detach_sent, instantiations, tot_sent):
    global pn
...
def substitute_in_conditionals(instantiations, conditionals):
    ...
    var_slots = [5, 14, 18]
    conditionals2 = []
    conditionals3 = copy.deepcopy(conditionals)
    total_num = []
    o = -1
    while o < len(instantiations) - 1:
        ...
        for i in num:
            for m in range(len(conditionals)):
                cond_sent = copy.deepcopy(conditionals3[m])
                if cond_sent[2] == i:
        ...
        list1 = make_new_conditionals(conditionals, num)
        for cond in list1:
            conditionals2.append(cond)
        if simul_sub:
            o += 1
    if len(total_num) < len(conditionals3):
        conditionals2 = delete_conditionals(conditionals3, total_num, conditionals2)

    return conditionals2


def delete_conditionals(conditionals3, total_num, conditionals2):
    # this combines the new conditionals with the old

    i = -1
    while i < len(total_num) - 1:
        i += 1
        try:
            j = findposinmdlistint(total_num[i], conditionals3, 2)
        except IndexError:
            bb = 8
        if j != -1:
            del conditionals3[j]

    for cond in conditionals3:
        conditionals2.append(cond)
...
def make_new_conditionals(conditionals, changed_conditionals):
    # this builds new strings within the conditional list

    global pn
    conditionals3 = []
    num = [34, 35, 32, 31, 30, 29]
    for i in changed_conditionals:
        u = findposinmdlistint(i, conditionals, 2)
        conditionals2 = copy.deepcopy(conditionals[u])
        conditionals2[26] = True  # this means its new and we need to print it
        pn += 1
        conditionals2[2] = pn
        prop_var_greek = conditionals2[36][5]
        prop_var_greek2 = prop_var_greek
        for j in num:
            for k in range(len(conditionals2[j])):
                atom_cond_sent = conditionals2[j][k]
                oldp = atom_cond_sent[42]
                old_nat = atom_cond_sent[0]
                temp_oldp = "(" + oldp + ")"
                oldp_greek = get_greek(conditionals2[36][6], temp_oldp)
                if atom_cond_sent[54]:
                    ...
                    if isinstance(conditionals2[n][0], list):
                        for m in range(len(conditionals2[n])):
                            if conditionals2[n][m][0] == abs_oldp:
                                conditionals2[n][m][0] = abs_newp
                    else:
                        if conditionals2[n][0] == abs_oldp:
                            conditionals2[n][0] = abs_newp
                    for m in range(len(conditionals2[38])):
                        if conditionals2[38][m] == oldp:
                            conditionals2[38][m] = list1[1]
                else:
                    prop_var_greek = prop_var_greek.replace(oldp_greek, oldp)
                    prop_var_greek2 = prop_var_greek2.replace(oldp_greek, old_nat)
        conditionals2[4] = prop_var_greek
        conditionals2[37] = prop_var_greek2
        list1 = get_ant_and_cond(prop_var_greek)
        conditionals2[6] = list1[0]
        conditionals2[7] = list1[1]
        conditionals3.append(conditionals2)

    return conditionals3
...
def print_general_object_properties(object_properties):
    ...
    for i in range(len(object_properties)):
        if object_properties[i][1] == "agen":
            ...
            if object_properties[i][2][0] == 'thing':
...
def print_new_conditionals(conditionals, tot_sent):
    # this adds the new conditionals to the tot_sent list
...
def print_instantiations(instantiations, tot_sent):
    # this adds the instantiations to the tot_sent list
    global pn
...
def print_object_properties(object_properties, tot_sent):
    tot_sent.append(["", '', '', '', '', '', '', '', ''])
    tot_sent.append(["", 'OBJECT PREDICATES', '', '', '', '', '', '', ''])
...
def determine_if_same_class(object_properties, idf_var, same_sent):
...
def infer_member(general_properties, general_groups, idf_var, \
...
def change_indef_attach_var(sent_parts, object_properties, k, instantiations):
    ...
    for list_properties in object_properties:
        set_detach = set(list_properties[2])
        list1 = set_attach.intersection(set_detach)
        if list1 == set_attach:
            det_var = list_properties[0]
            for sent in attach_sent_parts:
...
def get_detached_predicates(variable_type, detach_sent):
    ...
    for i in range(len(detach_sent)):
        ...
        ## possibly change this to not definite
        if s_variable_kind == 'agen':
            new_subj = alpha
        elif s_variable_kind != 'definite':
            # elif s_variable_kind == 'indef attach':
            new_subj = subj + "'"
        if o_variable_kind == 'agen':
            new_obj = alpha
        elif o_variable_kind != 'definite' and obj != "":
            # elif o_variable_kind == 'indef attach' and obj != "":
            new_obj = obj + "'"
        if o2_variable_kind == 'agen':
            new_obj2 = alpha
        elif o2_variable_kind != 'definite' and obj2 != "":
            # elif o2_variable_kind == 'indef attach' and obj2 != "":
            new_obj2 = obj2 + "'"
        ...
        sub_sent_parts = [alpha, t_value, relat, new_obj, relat2, new_obj2]
        if obj != "":
            obj_sent_parts = [new_subj, t_value, relat, alpha, relat2, obj2]
        if obj2 != "":
            obj2_sent_parts = [new_subj, t_value, relat, new_obj, relat2, alpha]
...
def get_general_object_properties(str1, object_properties, variable_kind, \
    ...
    if property in uninformative_properties or variable_kind == "indef detach":
        return object_properties
    d = findposinmd(str1, object_properties, 0)
    if kind != 'thing2' and sent_parts[2] == 'I':
        property = "$"
    if kind == 'thing2':
...
def get_object_properties(str1, object_properties, variable_kind, property, \
    ...
    if sent_kind[-1] == "q":
        return object_properties
    if kind == 'thing2':
        kind = 'thing'  # see kind exception for explanation
    uninformative_properties = ["I", "J", "H"]  # these are properties all object_properties have
    d = findposinmd(str1, object_properties, 0)
    if d == -1:
        if kind == "":
        ...
        for i in range(len(object_properties)):
            if object_properties[i][0] == str1:
                ...
                if property not in list_properties and property \
                    ...
                    list_sent_parts.append(sent_parts)
...
def get_attached_predicates(variable_type, conditionals, object_properties):
    ...
    for i in range(len(conditionals)):
        cond_num = conditionals[i][2]  # conditional number
        for j in num:
            if conditionals[i][j] != []:
                for k in range(len(conditionals[i][j])):
                    ...
                    if s_variable_kind in special_group:
                        object_properties = get_general_object_properties(subj, \
                                                                          ...
                                                                          spredicate, skind, sent_kind,
                                                                          sent_num, cond_num, sent_parts)
                    else:
                        object_properties = get_object_properties(conditionals[i][j][k][5], \
                                                                  object_properties, s_variable_kind, spredicate,
                    ...
                    if obj != "":
                        ...
                        if o_variable_kind in special_group:
                            ...
                            object_properties = get_object_properties(obj, object_properties,
                                                                      o_variable_kind, opredicate, okind,
                    ...
                    if obj2 != "":
                        if o2_variable_kind in special_group:
                            object_properties = get_general_object_properties(obj2, \
                                                                              ...
                                                                              o2predicate, 'thing', sent_kind, sent_num,
                                                                              cond_num, sent_parts)

                        else:
                            object_properties = get_object_properties(obj2, object_properties,
                                                                      o2_variable_kind, o2predicate, "thing",
                                                                      sent_kind, sent_parts)
...
def kind_exception(str1, variable_type, obj):
    # since everything belongs to the class 'whole' or 'part' these are not
    # genuine classes
    defn_attach = variable_type[4]
    exceptions = ['whole', 'part']
    # if obj in defn_attach:
    #     return 'thing'
...
def add_stan_sent(nonstandard, standard_cd, detach_sent, tot_sent):
                ...
                for h in range(len(pot_id)):
                    ...
                    if str2 in osent:
                        if all_sent[g][9] == "J" and (all_sent[g][14] == gen or \
                                                                  all_sent[g][14] == def_var or all_sent[g][
                            14] == indef):
                            pass
                        else:
                            for q in num:
...
def link_natural_sent_to_all_sent(conditionals, all_sent):
    ...
    for m in range(len(conditionals)):
        list7 = copy.deepcopy(conditionals[m])
        antecedent = []
        consequent = []
        third_d = []
        fourth_d = []
                                    ...
                                    if kind != 2:
                                        ...
                                        while s < len(conditionals[g][0]) - 1:
                                            ...
                                            if temp_detach_sent == conditionals[g][0][s][0] and \
                                                ...
                                                consistent = new_prop_sent(greek2, all_sent, "~", "con", \
                                                                           "EN", anc1, anc2, conditionals, g, candd,
                                                                           candd2)
                                                if not consistent:
                                                    return [False, conditionals]
                                                del conditionals[g]
                                        ...
                                        while s < len(conditionals[g][1]) - 1:
                                            ...
                                            if temp_detach_sent == conditionals[g][1][s][0] and \
                                                ...
                                                if str12 == 'e':
                                                    ...
                                                    str13 = "MT"
                                                consistent = new_prop_sent(greek2, all_sent, "~", "ant", \
                                                                           str13, anc1, anc2, conditionals, g, candd,
                                                                           candd2)
                                                if not consistent:
...
def demorgan(all_sent, prop_sent, conditionals, candd, candd2, kind, one_sent=False, str8="", anc1a="", rule="",
             conjt=[]):
    d = -1
    rop = False
    if one_sent:
                ...
                if conj not in conditionals[n][38] and iff not in conditionals[n][4] \
                    ...
                    if conj == str3:
                        ...
                        if str2 == str4 and str5 == "d":
                                        ...
                                        if candd[e][1] == list11[f][0]:
                                            if candd[e][2] == list11[f][1]:
                                                ...
                                                if list11 == []:
                                                    ...
                                                    if mc[0] == xorr:
                                                        ...
                                                        list1 = xorr_elim(conditionals, n, i, parent, \
                                                                          grandparent, whole_d, candd, candd2, conjt,
                                                                          prop_sent, pn, anc2, 1)
                                                        consistent = list1[0]
                                                        conditionals = list1[1]
                                                        if not consistent:
                                                        ...
                                                        if whole_d == grandparent:
                                                            list1 = new_disjunct(all_sent, str3, "", n, prop_sent, \
                                                                                 conditionals, candd, candd2, conjt,
                                                                                 anc1, anc3, anc4, anc5, 1)
                                                            conditionals = list1[1]
                                                        else:
                                                ...
                                                if str8.find("(") > -1 and (
                                                    ...
                                                    str8 = bad_paren(str8)
                                                list1 = new_disjunct(all_sent, str8, "", n, prop_sent, \
                                                                     conditionals, candd, candd2, conjt, anc1, anc2,
                                                                     None, None, 0, rule)
                                                consistent = list1[0]
                                                conditionals = list1[1]
                                                if not consistent:
                            ...
                            if str8.find("(") > -1 and str8.find(idisj) > -1:
                                str8 = bad_paren(str8)
                            list1 = new_disjunct(all_sent, str8, "", n, prop_sent, conditionals, candd, candd2, conjt,
...
def detach_wo_instantiation(greek2, sent, all_sent, prop_sent, candd, candd2, conditionals, prop_name, disjuncts,
                            tot_sent, kind, negat):
    global conc
    global sn
    global rel_conj
