...
def define(tot_sent, all_sent, idf_var, dv_nam,words,rep_rel,identities):
    ...
    atoms2 = [['moment','T',14],['relationship','I',5],['point','S',14],['number','AN',14],\
...
def build_sent(list1):
    ...
    str1 = "("
    num = [11,1,2,47,3,4,5,66,67,35,48,59,6,8,9,7,48,12,10,13,14,36,60,63,49,15,16,17,18,\
            ...
            if all_sent[m][46] != "x":
                ...
                for i in num:
                    ...
                    if k == 0 and all_sent[m][i] != None and kind == 0:
                        ...
                        dummy = that(all_sent,m,i,tot_sent,def_sent,dv_nam,words,idf_var)
...
def that(all_sent,m,i,tot_sent,def_sent,dv_nam,words,idf_var):
    ...
    for j in num:
        if j == i:
            bool1 = True
        if all_sent[m][j] != None and bool1 and j!=i:
    ...
    all_sent[m][i] == None
    if i == 7:
        all_sent[m][14] = new_var
    elif i == 61:
        if all_sent[m][14] == 'it'+up:
            all_sent[m][61] = 'that'+uc
        all_sent[m][64] = new_var
    elif i == 62:
        if all_sent[m][14] == 'it'+up:
            all_sent[m][62] = 'that'+uc
        all_sent[m][65] = new_var

    str1 = build_sent(list2)
    list2[0] = str1
    all_sent.append(list2)
    str1 = remove_outer_paren(str1)
    dv_nam.append([new_var,str1])
    all_sent[m][46] = "x"
...
def abb_change(list5, already_checked,all_sent,def_sent,i,match_dv,rename,j,def_con,\
    ...
    for t in range(len(all_sent)):
        ...
        if bool1:
            pass
        elif t not in already_checked and all_sent[t][46] != "x":
            for u in list5:
                str5 = all_sent[t][u]
                asent = all_sent[t][0]
                dsent = def_sent[i][0]
...
def categorize_words(words,str2,idf_var,all_sent,kind=1,first=False,snoun="",snum=""):
    ...
    while i < g -1:
        ...
        if word == ' ':
            ...
            list1_cat[59] = word
            list2.append(59)
        elif word == 'that'+uc and list1_cat[7] == None:
             list1_cat[7] = word
             list2.append(7)
...
def cat_atoms(j,i,list,members,basic_objects,str1,bo2,words,consq,rel,basic_cat):
    ...
    if list[i][8] == None:
        list[i][8] = ""
    num = [5,14,18,22]
    if subj == "x" and relat == 'IG' and obj == 'z' and j == 5:
        bb = 8
    ...
    if str1 != "" and orelata in gen_var:
        ...
        str9 = "cn"
        consq.append(sent)
    elif relat == 'IG' and obj in gen_var:
        str5 = 'ir'
    else:
        for k in num:
...
def convert(list1):
    ...
    list3 = []
    for i in range(len(list2)):
        list3.append([list2[i],more_prop[i]])
        oprop = oprop.replace(list2[i],more_prop[i])
    return [oprop,list3]
...
#rajiv - use these numbers for the progress bar
    for k in range(strt,stp):
        ...
        tot_sent = identity(all_sent,tot_sent,basic_objects,words,candd,candd2,\
